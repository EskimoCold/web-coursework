...
export const CategoryForm: React.FC<Props> = ({label, submit, modify, placeholder}: Props) => {
  ...
  const [description, setDescription] = useState(placeholder ? placeholder.category.description : '');
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!placeholder) setCategories(prev => [...prev, newCategory]);
        ...
        setCategories(prev => prev.map(cat => cat.id === placeholder.category.id ? newCategory : cat));
  ...
  return (
    ...
    >
      ...
      <button className={`cat-button ${type ? 'green' : 'red'}`} type="button" onClick={() => setType(!type)}>{type ? 'Доход' : 'Расход'}</button>
      
      <div className='cat-form-buttons'>
        <button className='cat-button submit' type="submit" disabled={!isSubmittable}>{submit}</button>
        {modify && <button className='cat-button red' type="reset">Удалить</button>}          
      </div>
    </form>
  );
};